                ...
                Dir.chdir(pkg.srcdir) do
                    begin
                        ...
                        if not gem 
                            ...
                            if !system("rake clean > #{File.join(OBS_LOG_DIR, logname)} 2> #{File.join(OBS_LOG_DIR, logname)}")
                                if !system("rake dist:clean > #{File.join(OBS_LOG_DIR, logname)} 2> #{File.join(OBS_LOG_DIR, logname)}")
                                    if !system("rake clobber > #{File.join(OBS_LOG_DIR, logname)} 2> #{File.join(OBS_LOG_DIR, logname)}")
                                        raise "Debian: failed to cleanup ruby package"
                                    end
                                end
                            end
                            ...
                            if !system("rake gem >> #{File.join(OBS_LOG_DIR, logname)} 2>> #{File.join(OBS_LOG_DIR, logname)}")
                                if !system("rake dist:gem >> #{File.join(OBS_LOG_DIR, logname)} 2>> #{File.join(OBS_LOG_DIR, logname)}")
