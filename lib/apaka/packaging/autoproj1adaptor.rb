            ...
            def pkgmanifest_by_name(package_name)
                if !@pkg_manifest_cache[package_name]
                    begin
                        ...
                        raise RuntimeError, "Apaka::Packaging::Autoproj1Adaptor: failed to load manifest for '#{package_name}' -- #{e}"
                    end
                end
                ...
                while true
                    ...
                    all_packages.each do |pkg_name|
                        begin
                            ...
                            raise RuntimeError, "Apaka::Packaging::Autoproj1Adaptor: failed to load manifest for '#{pkg_name}' -- #{e}"
                        end
