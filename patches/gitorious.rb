module Autoproj
	...
	if Autobuild::Importer.respond_to?(:fallback) && options[:fallback_to_http]
	    block = Proc.new do |package, importer|
                ...
                if importer.kind_of?(Autobuild::Git) && importer.repository =~ root_rx && importer.repository !~ /^http/
                    Autoproj.warn "import from #{importer.repository} failed." 
                    Autoproj.warn "waiting for 60 seconds and retrying... "
		    sleep(60)
		    importer
                end
            end
	    Autobuild::Importer.fallback(&block)
	    Autobuild::Importer.fallback(&block)

	    Autobuild::Importer.fallback do |package, importer|
                root_rx = /^(?:http:\/\/git\.|git:\/\/|git@)#{Regexp.quote(base_url)}:?/
                if importer.kind_of?(Autobuild::Git) && importer.repository =~ root_rx && importer.repository !~ /^http/
                    Autoproj.warn "import from #{importer.repository} failed, falling back to using http for this package on #{base_url}"
                    Autoproj.warn "waiting for 60 seconds ... "
		    sleep 60 
                    base_replace_string = "http://git.#{base_url}"
                    if package.importer.kind_of?(Autobuild::Git) && package.importer.repository =~ root_rx
                        if package.importer.repository =~ /^git@/
                            replace_string = "#{base_replace_string}/"
                        else
                            replace_string = base_replace_string
                        end
                        package.importer.repository.gsub!(root_rx, replace_string)
                    end
